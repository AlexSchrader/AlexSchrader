## 👋 Hi, I'm Alex Schrader

Computer Programming & Development student at Wake Tech (Associate's, Dec. 2026) and ITS-certified in Python. I like building real things — web apps, iOS apps, and the occasional tool to solve a problem in front of me.

### 🔧 What I'm working on
- **[Schrader Finance](https://schrader-finance.vercel.app)** — personal finance web app for budgeting and net-worth tracking · React 18 + Vite, deployed on Vercel
- **Squawk Flight Log** — native iOS logbook for pilots (flights, squawks, currency) · App Store target
- **Club19** — iOS app, converting from a PWA to native

### 🛠️ Tech
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

### 🌱 Currently learning
Deepening C++, Java, and Python · iOS/Swift development · Japanese (日本語) on the side

### ⚡ Outside the editor
Basketball, cars, motorcycles, and anything mechanical. Always taking something apart to see how it works.

### 📫 Reach me
- 📧 acs.schrader123@gmail.com
- 🎓 aschrader@my.waketech.edu
